...
def cluster(data, k=30, directed=False, prune=False, min_cluster_size=10, jaccard=True,
    ...
    if prune:
        logger.info("Setting directed=False because prune=True")
        directed = False
    ...
    if isinstance(data, sp.spmatrix) and data.shape[0] == data.shape[1]:
        logger.info("Using neighbor information from provided graph, rather than computing neighbors directly")
